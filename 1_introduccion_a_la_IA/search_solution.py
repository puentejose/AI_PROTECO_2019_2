...
def depthFirstSearch(problem):
    ...
    pila = util.Stack() #Usamos función stack declarada en util.py (Importada al principio del archivo)

    # Inicializamos lista que contendrá estados recorridos
    estadosRecorridos = []

    # Lista que almacenará los pasos tomados para llegar al estado, lo que se retonará al final del algoritmo como solución
    plan = []

    # Un nodo consiste en una tupla con las coordenadad y el movimiento realizado para llegar ahí: ((x, y), 'ACCION')
    ...
    while( not pila.isEmpty() ):
        ...
        estadoActual, plan = pila.pop()

        # Si el estado actual no es un estado meta...
        if not problem.isGoalState(estadoActual):

            # Si el estado actual no se encuentra ha sido recorrido..[]
            if estadoActual not in estadosRecorridos:

                # Marcamos el estado actual como recorrido agragandolo a la lista estadosRecorridos
                ...
                for hijo in problem.getSuccessors(estadoActual):
...
def breadthFirstSearch(problem):
    ...
    # Lista que almacena los nodos recorridos
    estadosRecorridos = []
    plan = []
    ...
    while( not cola.isEmpty() ):
        ...
        estadoActual, plan = cola.pop()

        # Si el estado actual no es un estado meta...
        if not problem.isGoalState(estadoActual):

            # Si el estado actual no ha sido recorrido...
            if estadoActual not in estadosRecorridos:
